...
static inline void hash_table_bucket_lock(struct hash_table *t, unsigned int n)
{
	return (mutex_lock(&(t->bucket_locks[n])));
}

static inline void hash_table_bucket_unlock(struct hash_table *t, unsigned int n)
{
	return (mutex_unlock(&(t->bucket_locks[n])));
}

static inline void hash_table_lock(struct hash_table *t)
{
	return (mutex_lock(&(t->lock)));
}

static inline void hash_table_unlock(struct hash_table *t)
{
	return (mutex_unlock(&(t->lock)));
}

static inline int hash_table_bucket_locked(struct hash_table *t, unsigned int n)
{
	return (mutex_trylock(&(t->bucket_locks[n])) == EINTR);
}

static inline int hash_table_locked(struct hash_table *t)
{
	return (mutex_trylock(&(t->lock)) == EINTR);
}
				  ...
				  keycmp_ptr keycmp)
{
	...
	if ((h->bucket_locks =
	     (struct mutex *) kmalloc(sizeof(struct mutex) * b, GFP_KERNEL)) == NULL)
		return -1;

	for (--b; b != 0; --b) {
		hash_entry_init(&(h->table[b]), NULL, 0);
		mutex_init(&h->bucket_locks[b]);
	}

	hash_entry_init(&(h->table[0]), NULL, 0);
	mutex_init(&h->bucket_locks[0]);
